  ...
  img.directive('imgShow',['$compile','$timeout','$ionicGesture',function($compile, $timeout, $ionicGesture) {
    return {
      ...
      link:function($scope,$element,$attrs) {
        ...
        // 图片对象
        var target_ele = $element[0].querySelector('img');

        // target_ele.style.translateY='100px';
        // target_ele.style.width='100px';
        // target_ele.style.height='100px';
        var initScale,
            // 开始拖拽距离
            startX = parseInt(target_ele.style.left),
            startY = parseInt(target_ele.style.top);
        new AlloyFinger(target_ele,{
          ...
          pressMove: function (evt) {
            target_ele.style.left = startX + evt.deltaX + 'px';
            target_ele.style.top = startY + evt.deltaY + 'px';
            // startX = parseInt(target_ele.style.left);
			      // startY = parseInt(target_ele.style.top);
              // var cr = self.img.getBoundingClientRect();
              // var boxOffY = (document.documentElement.clientHeight - self.height)/2;
