...
angular.module('App').controller('redPacketCtl',function(enterViewLoad,appUtils,$ionicPopup,$localStorage,$ionicModal,$rootScope,$timeout,$scope,$state,$stateParams,$http,$Factory,$ionicLoading){
	$scope.back=function(){
		appUtils.back();
	}
	...
	$ionicModal.fromTemplateUrl('read_packer_password_modal', {
	    scope: $scope,
	...
	$scope.inputPassword=function(){
		$('.read-packer-password-modalview .password').on('input',function(){
			var value = $(this).val();
			var length = value.length;
			$('.read-packer-password-modalview .showView').find('li').each(function(i){
				if(i < length){
