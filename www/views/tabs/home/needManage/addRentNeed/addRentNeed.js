angular.module('App')
.controller('addRentNeedCtl',function(appUtils,$addRentNeedData,enterViewLoad,$ionicScrollDelegate,$ionicModal,$stateParams,$timeout,$state,$Factory,$http,$scope){
	$scope.back=function(){
		appUtils.back();
	}
	...
	$scope.initRange=function(){
		$("#add_need_rent_price_range").ionRangeSlider({
			type: "double",
			min: 0,
			max: 20000,
			from: $scope.needInfo?$scope.needInfo.MinPrize:0,
			to: $scope.needInfo?$scope.needInfo.MaxPrize:20000,
			postfix: "元/月"
		});
		$scope.rangePriceData = $("#add_need_rent_price_range").data("ionRangeSlider");
		$("#add_need_rent_space_range").ionRangeSlider({
			type: "double",
			min: 0,
			max: 1000,
		...
		$scope.rangeSpaceData = $("#add_need_rent_space_range").data("ionRangeSlider");
	}
	...
	$scope.$on('$ionicView.enter',function(){
		$scope.addSubData=$addRentNeedData.data;
	...
	$scope.initData=function(){
		...
		$.scrEvent({
			data: districtList,   
			evEle: '.rent-need-district',            
			title: '请选择区域',            
			defValue: defDsitrict,             
			afterAction: function (data) {   
				$(".rent-need-district").val(data);
	...
	$scope.save=function(){
		...
		if($scope.pageType==4||$scope.pageType==6||$scope.pageType==8){
		...
		}else if($scope.pageType==3||$scope.pageType==10){
			$scope.addSubData.RoomType=null;
			$scope.addSubData.HallType=null;
			$scope.addSubData.BathType=null;
		}
...
.factory('$addRentNeedData', [function () {
	/* util 构造函数 */
	var Util = function () {	
	};
	/* util 原型对象 */
