...
// 海量数据卡顿
    ...
    function Cluster(markerClusterer){
        ...
        this._labels = [];
        this._clusterMarker = new BMapLib.TextIconOverlay(this._center, {name:'共找到',value : this._markers.length+'套房源'}, {"styles":this._markerClusterer.getStyles()});
...
// 海量数据 卡顿问题 注释以下源码 MarkerClusterer.prototype._createClusters添加代码
        // var len = this._markers.length;
    ...
    Cluster.prototype.addLabel = function (marker) {
        ...
        var content = '<span style="color:#333"><i class="fa fa-map-marker"></i></span>'
                    +'<p data-id="'+marker.id+ '" data-city="'+marker.city+ '" data-text="'+marker.communityName+'" style="padding:0px 13px;text-align:center;line-height:25px;margin:0;">'+marker.communityName+'('+marker.housecount+'套)'+'</p>';
    ...
    Cluster.prototype.updateClusterMarker = function () {
        ...
        var _customMarkerText=''
   
        if (zoom<=7) {
            _customMarkerText = this._markers[0].province;
        }else if(zoom>7&&zoom<=9){
            _customMarkerText = this._markers[0].city;
        }else if(zoom > 9 && zoom <= 11){
            _customMarkerText = this._markers[0].district;
        }else if (zoom > 11) {
            for (var i=0;i<this._markers.length;i++){
                if(this._markers[i].street) {
                    _customMarkerText = this._markers[i].street;
                    break;
                }
            }
            if (!_customMarkerText){
                _customMarkerText = this._markers[0].city +"片区";
            }
        }
        // console.log(_customMarkerText)
        this._clusterMarker.setText(
            '<div style="width: 80px;height:80px;line-height: 80px;"><span style="display:inline-block;vertical-align:middle;line-height:20px;">'+_customMarkerText
            +'<br/>'+
            this._markers.length+'个小区'
            +'<span></div>'
            );
