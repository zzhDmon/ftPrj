angular.module('App').controller('needBuyListCtl',function(enterViewLoad,appUtils,$interval,$ionicModal,$ionicScrollDelegate,$timeout,$http,$Factory,$state,$stateParams,$scope,$rootScope,$ionicLoading,$document,$ionicHistory){
	$scope.back=function(){
		appUtils.back();
	}
	...
	function _loadData(ifloadmore){
		$http.get($Factory.HouseNeed.publicquery.url,{
			...
			if(ifloadmore){
				...
				$scope.needBuyArr=$scope.needBuyArr.concat(resData.data);
				$scope.$broadcast('scroll.infiniteScrollComplete');
			}else{
				$scope.queryData.pagenum=1
